...
export const createProductController = async (req, res) => {
  try {
    ...
    const products = new productModel({ ...req.fields, slug: slugify(nama) });
    if (photo) {
...
export const relatedProductController = async (req, res) => {
  try {
    ...
    res.status(400).send({
      success: false,
      message: "error getting realted product",
      error,
    });
  }
};
...
//get detail product
export const getDetailProductController = async (req, res) => {
  try {
    const { id } = req.params
    const products = await productModel
